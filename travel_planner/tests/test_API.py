...
class CustomUserAPIViewTest(APIBaseTest):
    ...
    def test_list_users_as_admin(self):
        ...
        # We expect 3 users: admin, regular_user, other_user + 1 internal Django superuser = 4 users. (created because of ContentType)
...
class DestinationAPIViewTest(APIBaseTest):
    ...
    def test_list_destinations_unauthenticated(self):
        ...
        self.assertEqual(len(response.data['results']), 2) # Both user_destination and other_user_destination
...
class TravelPlanAPIViewTest(APIBaseTest):
    ...
    def test_retrieve_own_private_travel_plan(self):
        ...
        if response.status_code != status.HTTP_200_OK:
            ...
            print(f"Status Code: {response.status_code}")
            print(f"Response Data (Errors): {response.data['results']}")
            print("--- END DEBUG INFO ---")
        # --- END DEBUG INFO ---
...
class TravelPlanDestinationAPIViewTest(APIBaseTest):
    ...
    def test_list_tpd_own_plan_authenticated_user(self):
        ...
        self.assertEqual(response.data['results'][0]['destination_detail']['name'], 'Paris') 
    ...
    def test_list_tpd_other_plan_authenticated_user_empty(self):
        ...
        self.assertEqual(response.data['results'][0]['destination_detail']['name'], 'Paris')
        self.assertEqual(response.data['results'][0]['travel_plan_display'], str(self.user_plan)) # Check string representation
...
class ActivityAPIViewTest(APIBaseTest):
    ...
    def test_list_activities_own_plan_authenticated_user(self):
        ...
        self.assertEqual(len(response.data['results']), 1)
        self.assertEqual(response.data['results'][0]['name'], 'Visit Museum')
        # Check nested destination detail
        self.assertEqual(response.data['results'][0]['destination_detail']['name'], 'Museum')
        # Check travel plan display string
        self.assertEqual(response.data['results'][0]['travel_plan_display'], str(self.user_plan))
    ...
    def test_list_activities_other_plan_authenticated_user_empty(self):
        ...
        response = self.client.get(reverse('activity-list-create'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']), 1) # Still 1, not 2, because other's Activity is filtered out
...
class CommentAPIViewTest(APIBaseTest):
    ...
    def test_list_comments_authenticated_user(self):
        ...
        comment_texts = {p['text'] for p in response.data['results']}
        ...
        if response.data:
            self.assertIn('user_username', response.data['results'][0])
            self.assertIn('content_object_display', response.data['results'][0])
